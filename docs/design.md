...
- **完全無料**。App内課金・広告・サブスクリプションは実装しない（当初は14日試用＋買い切りで設計したが、公開方針の変更で全面的に外した）
...
| 課金 | 試用14日→買い切りでTier3解放 | **無し（完全無料）**。StoreKitに依存しない |
...
App内課金・サブスクリプション・広告のいずれも実装しない。収録している全項目が
最初から出題対象で、機能や範囲が制限される期間もない。

そのため次を**持たない**。

- StoreKit への依存（`AccessRights` / `TrialManager` / `Entitlements` / `PaywallView`）
- `Products.storekit`
- 項目ごとの無料/有料フラグ（`isFree`）

当初は参考元と同じ「14日試用 → 買い切り1商品」で設計・実装していた。無料公開に決めた
時点で丸ごと外している。再導入が必要になったら git 履歴の `AccessRights` 周辺を参照する
（無料範囲として選定した単語147語・助動詞28項目の内訳もそこに残っている）。

購入・復元のための通信も無くなったため、**アプリはネットワーク通信を一切行わない**。
プライバシーポリシー（`docs/site-content.md`）もこの前提で書いてある。
